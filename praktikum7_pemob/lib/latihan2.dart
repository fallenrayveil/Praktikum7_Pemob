...
// event parent
abstract class NegaraEvent {}

// event pilih negara
class PilihNegara extends NegaraEvent {
  final Negara negara; // atribut negara

  PilihNegara(this.negara); // konstruktor PilihNegara
}

// event state
abstract class NegaraState {}

// event negara terpilih
class NegaraSelected extends NegaraState {
  final Negara negara; // atribut negara

  NegaraSelected(this.negara); // konstruktor NegaraSelected
}
...
class NegaraCubit extends Cubit<NegaraState> { // class NegaraCubit
  NegaraCubit() : super(NegaraSelected(daftarNegara[0])); // super NegaraSelected

  void pilihNegara(Negara negara) => emit(NegaraSelected(negara)); // emit NegaraSelected
}
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: BlocProvider( // BlocProvider
        create: (context) => NegaraCubit(), // create NegaraCubit 
...
class PilihanNegaraDropdown extends StatelessWidget {
  @override
  Widget build(BuildContext context) { 
    return BlocBuilder<NegaraCubit, NegaraState>(  // BlocBuilder
      builder: (context, state) {
        if (state is NegaraSelected) { // jika state NegaraSelected 
          final negara = state.negara; // negara dari state negara
          return DropdownButton<Negara>( // return DropdownButton Negara
            value: negara,
            onChanged: (newValue) { 
          ...
          return CircularProgressIndicator(); // loading jika negara belum terpilih
        }
      },
    );
  }
    ...
    return BlocBuilder<NegaraCubit, NegaraState>( // BlocBuilder
      builder: (context, state) { 
        if (state is NegaraSelected) { // jika state NegaraSelected
          ...
          return Container(); // tidak menampilkan apapun jika negara belum terpilih
        }
      },
    );
  }
