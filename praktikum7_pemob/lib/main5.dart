...
class PopulasiTahun {
  ...
  int populasi; //perlu int
  charts.Color barColor; //untuk menambahkan warna bar
  PopulasiTahun(
      {required this.tahun, required this.populasi, required this.barColor}); //constructor
}

class Populasi {
  List<PopulasiTahun> ListPop = <PopulasiTahun>[]; //list untuk menampung data

  Populasi(Map<String, dynamic> json) { //constructor
    // isi listPop disini
    var data = json["data"]; //ambil data
    for (var val in data) { //looping data
      var tahun = val["Year"]; //ambil tahun
      var populasi = val["Population"]; //ambil populasi
      var warna =
          charts.ColorUtil.fromDartColor(Colors.green); //satu warna dulu
      ListPop.add(
          PopulasiTahun(tahun: tahun, populasi: populasi, barColor: warna)); //tambahkan ke list
    }
  }

  //map dari json ke atribut
  factory Populasi.fromJson(Map<String, dynamic> json) { //method
    return Populasi(json); //return
  }
}
...
class PopulasiChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    List<charts.Series<PopulasiTahun, String>> series = [ //series
      charts.Series(
          id: "populasi", 
          data: listPop, 
